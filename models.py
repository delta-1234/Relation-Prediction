...
class SpKBGATModified(nn.Module):
    ...
    def forward(self, Corpus_, adj, batch_inputs, train_indices_nhop):
        ...
        if len(train_indices_nhop) == 0:
            edge_list_nhop = torch.cat((train_indices_nhop.unsqueeze(-1), train_indices_nhop.unsqueeze(-1)), dim=1).t()
            edge_type_nhop = torch.cat((train_indices_nhop.unsqueeze(-1), train_indices_nhop.unsqueeze(-1)), dim=1)
        else:
            print(train_indices_nhop)
